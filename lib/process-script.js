...
let message_listeners = {
    result_method_for_url: data => {
        ...
        if (window.top && window.top !== window.self && processed_documents.has(window.top.document)) {
    ...
    update_applied_methods: () => {
        forEachNsIDOMWindow(window => {
            if (!(window.document))
                return;
            load_into_window(window, null, SKIP_PENDING_CHECK);
        })
    },
    update_options: msg => {
        forEachNsIDOMWindow(window => {
...
function load_into_window(window, no_defer, skip_pending_check) {
    // don't try to process already dead windows
    if (Components.utils.isDeadWrapper(window)) {
        return;
    }

    // #15 check if tab that contains this window not in pending state otherwise do nothing
    if (!skip_pending_check && window == window.top && chrome_process) {
        if (isFennec) {
    ...
    if (window.document.documentURI === 'about:blank' && !no_defer) {
        setTimeout(() => { load_into_window(window, true, skip_pending_check) }, 1000);
        return;
    }
