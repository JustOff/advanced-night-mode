...
function update_global_sheet(unload) {
    ...
    if (!unload) {
        ...
        if (typeof gDevTools !== "undefined") {
          gDevTools.emit("pref-changed", { pref: "devtools.theme", newValue: "dark", oldValue: "light" });
        }
    } else {
        Object.keys(fx_prefs).forEach(pref => about_config.reset(pref));
        if (typeof gDevTools !== "undefined") {
          gDevTools.emit("pref-changed", { pref: "devtools.theme", newValue: "light", oldValue: "dark" });
        }
    }
}
